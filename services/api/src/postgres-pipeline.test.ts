...
test("pipeline, stages and deals are configurable with optimistic versions", async () => {
  ...
  try {
    ...
    const deal = await store.createDeal("ws_demo", { contactId: contactId!, pipelineId: pipelineId!, stageId: pipeline.stages[0].slug, title: "Новая сделка", amount: 12500 }); dealId = deal.id; assert.equal(deal.amount, 12500);

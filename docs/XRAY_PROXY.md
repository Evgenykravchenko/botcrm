...
Для обычных обновлений используйте эквивалентную npm-команду, которая всегда
подключает оба дополнительных Compose-файла:

```bash
npm run prod:tailscale:xray:up
```

Не обновляйте такую установку через `prod:up` или `prod:tailscale:up`: Docker
пересоздаст `api` и `worker` без переменных прокси. В результате входящие
события от внешнего бота продолжат появляться в CRM, но проверка коннектора,
загрузка Telegram-аватаров и другие исходящие запросы к API канала перестанут
работать.

...
import { AlertTriangle, ArrowUpRight, BarChart3, Bot, BookOpen, Calendar, Check, CheckCheck, ChevronDown, Circle, Columns3, Copy, Database, ExternalLink, Eye, GripVertical, Inbox, Mail, Megaphone, Menu, MessageCircle, Mic, Moon, MoreHorizontal, Paperclip, Pause, Phone, Plus, RefreshCw, Search, Send, Settings, ShieldCheck, SlidersHorizontal, Smile, Sun, Tag, User, Users, Workflow, X, Zap } from "lucide-react";
...
export function BotCRM() {
  ...
  async function createCampaign(input: { name: string; channel: Channel; content: string; buttons?: ApiCampaignButton[]; mediaIds?: string[]; segmentId?: string; scheduledAt?: string; timeZone?: string }) {
    try {
      const created = await botcrmApi.createCampaign(input);
      setCampaignItems((items) => [toCampaignCard(created), ...items]);
      setCampaignComposer(false);
      notify(created.status === "scheduled" ? "Рассылка запланирована" : "Рассылка сохранена как черновик");
    } catch (error) {
  ...
  return <div className="app-shell" data-theme={dark ? "dark" : "light"}>
    ...
    <main className="main-area">
      ...
      {section === "campaigns" && <CampaignsView canManage={canSupervise} items={campaignItems} onCreate={() => setCampaignComposer(true)} onStart={startCampaign} onPause={pauseCampaign} onCancel={cancelCampaign} actionId={startingCampaignId} />}
    ...
    {campaignComposer && canSupervise && <CampaignModal segments={segments} close={() => setCampaignComposer(false)} launch={createCampaign} />}
...
function CampaignsView({ canManage, items, onCreate, onStart, onPause, onCancel, actionId }: { canManage: boolean; items: CampaignCardItem[]; onCreate: () => void; onStart: (id?: string) => void; onPause: (id?: string) => void; onCancel: (id?: string) => void; actionId: string | null }) {
  ...
  const [recipientCampaign,setRecipientCampaign]=useState<CampaignCardItem|null>(null); const [recipients,setRecipients]=useState<ApiCampaignRecipient[]>([]); const [recipientsLoading,setRecipientsLoading]=useState(false);
  ...
  const sentTotal=filtered.reduce((sum,item)=>sum+(item.sentCount??0),0); const deliveredTotal=filtered.reduce((sum,item)=>sum+(item.deliveredCount??0),0); const readTotal=filtered.reduce((sum,item)=>sum+(item.readCount??0),0); const failedTotal=filtered.reduce((sum,item)=>sum+(item.failedCount??0),0);
  ...
  return <div className="page-scroll">
    <div className="stats-strip campaign-stats"><MetricMini label="Отправлено" value={new Intl.NumberFormat("ru-RU").format(sentTotal)}/><MetricMini label="Доставлено" value={sentTotal?`${(deliveredTotal/sentTotal*100).toFixed(1)}%`:"—"}/><MetricMini label="Прочитано" value={new Intl.NumberFormat("ru-RU").format(readTotal)}/><MetricMini label="Ошибки" value={new Intl.NumberFormat("ru-RU").format(failedTotal)} down={failedTotal>0}/></div>
    ...
    <div className="campaign-grid">{filtered.map((item)=>{const busy=actionId===item.id;const canStart=["draft","scheduled","paused"].includes(item.rawStatus);const canPause=item.rawStatus==="running";const canCancel=!["completed","cancelled"].includes(item.rawStatus);return <article className="campaign-card" key={item.id||item.name}><div className="campaign-icon"><Megaphone size={21}/></div><div className="campaign-main"><div><h3>{item.name}</h3><span className={`campaign-status ${item.tone}`}><i/>{item.status}</span></div><p><span>{item.channel}</span><i/>{item.segment}</p><div className="campaign-numbers"><label><span>Аудитория</span><b>{item.audience}</b></label><label><span>Доставлено</span><b>{item.delivered}</b></label><label><span>Ответы</span><b>{item.replies}</b></label></div></div><div className="campaign-card-actions">{canManage&&canStart&&<button className="button primary campaign-start" disabled={!item.id||busy} onClick={()=>onStart(item.id)}>{busy?<RefreshCw size={15} className="spin"/>:<Send size={15}/>} {busy?"Обработка…":item.rawStatus==="paused"?"Продолжить":"Запустить"}</button>}{canManage&&canPause&&<button className="button secondary campaign-start" disabled={!item.id||busy} onClick={()=>onPause(item.id)}><Pause size={15}/>{busy?"Обработка…":"Пауза"}</button>}{canManage&&canCancel&&<button className="icon-button campaign-cancel" aria-label={`Отменить рассылку ${item.name}`} disabled={!item.id||busy} onClick={()=>onCancel(item.id)}><X size={17}/></button>}{canManage && <button className="icon-button" aria-label={`Получатели рассылки ${item.name}`} onClick={()=>void showRecipients(item)}><Eye size={18}/></button>}</div></article>;})}{canManage && <button className="new-campaign-card" onClick={onCreate}><div><Plus size={23}/></div><b>Создать рассылку</b><span>Выберите сегмент и канал</span></button>}</div>
...
function CampaignModal({ segments, close, launch }: { segments: ApiSegment[]; close: () => void; launch: (input: { name: string; channel: Channel; content: string; buttons?: ApiCampaignButton[]; mediaIds?: string[]; segmentId?: string; scheduledAt?: string; timeZone?: string }) => Promise<void> }) {
  const [name, setName] = useState("Август · новый запуск");
  const [channel, setChannel] = useState<Channel>("telegram");
  const [content, setContent] = useState("Здравствуйте, {{first_name}}! Подготовили для вас персональное предложение. Ответьте на это сообщение, и менеджер расскажет детали.");
  const [buttons, setButtons] = useState<ApiCampaignButton[]>([]);
  ...
  const [scheduledAt, setScheduledAt] = useState(() => defaultCampaignSchedule("Europe/Moscow"));
  const [submitting, setSubmitting] = useState(false);
...
function ConnectorModal({ close, created }: { close: () => void; created: () => Promise<void> }) {
  ...
  function credentials(): Record<string, unknown> {
    ...
    if (channel === "vk") return { accessToken: primarySecret, apiVersion: "5.199", confirmationSecret: webhookSecret, confirmationCode: verificationValue };
